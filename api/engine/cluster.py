...
def create_clusters(user_id, entry_count, new_embeddings, new_entry_id):
    ...
    count = entry_count
    new_cluster_count = 2*int(sqrt(count))
    print(new_cluster_count)

    # check if we can stick with the same number of clusters
    #if new_cluster_count == cluster_count:
    # NOTE: only true if KMeans because KMeans is the only clustering algorithm with a predict() function
    if CLUSTERING_ALGORITHM == KMeans and new_cluster_count == cluster_count:
    ...
    # clusters need regenerated
        ...
        try:
            ...
            for embedding in embedding_queryset:
                ...
                if avg is None:
                    ...
                    average_dict[entry_id] = [avg[0] + pickle.loads(embedding.embedding), avg[1] + 1]
        ...
        clusterer = SpectralClustering(n_clusters=new_cluster_count)
